...
export const CardPageComponent = () => {
    ...
    useEffect(() => {
        let fetchData = async() => {
            let responce = await fetch(`https://free-to-play-games-database.p.rapidapi.com/api/game?id=${id}`, {
              headers: {
        ...
        <>
            <div>
                {game ? (
                    ...
                    <span className="gamePage-shortDescription">{game.short_description}</span>
                    <div className="gamePage-content-wrapper">
                    <Carousel slide={true} className="carousel">
                        {game.screenshots.map((screenshot, index) => (
                                <Carousel.Item key={index} interval={3000}>
                                    <img src={screenshot.image} alt={`Screenshot ${index + 1}`} className="carousel-img"/>
                                </Carousel.Item>
                        ))}
                    </Carousel>
                         ...
                         {game.minimum_system_requirements && (
                            ...
                            <div className="gamePage-requirements-list gamePage-list">
                                <div className="gamePage-list-div">
                                    ...
                                    <p className="gamePage-list-p">{game.minimum_system_requirements.os}</p>
                                </div>
                                <div className="gamePage-list-div">
                                    <p className="gamePage-list-h4">Processor:</p>
                                    <p className="gamePage-list-p">{game.minimum_system_requirements.processor}</p>
                                </div>
                                <div className="gamePage-list-div">
                                    <p className="gamePage-list-h4">Memory:</p>
                                    <p className="gamePage-list-p">{game.minimum_system_requirements.memory}</p>
                                </div>
                                <div className="gamePage-list-div-last">
                                    <p className="gamePage-list-h4">Graphics:</p>
                                    <p className="gamePage-list-p">{game.minimum_system_requirements.graphics}</p>
                                </div>
                                <div className="gamePage-list-div-last">
                                    <p className="gamePage-list-h4">Storage:</p>
                                    <p className="gamePage-list-p">{game.minimum_system_requirements.storage}</p>
                                </div>
                            </div>
                        </div>
                         )}
                    ...
                    <div className="gamePage_link">
                        <a href={game.game_url} target="_blank">Game profile</a>
                    </div>
                </div>
